...
function renderProducts() {
    ...
    products.forEach(p => {
        ...
        card.className = "group recipe-card rounded-[2.5rem] p-8 shadow-lg text-center cursor-pointer transition-all duration-300 hover:shadow-2xl flex flex-col items-center relative overflow-hidden";
        card.onclick = () => openModal(p);
        card.innerHTML = `
            <div class="recipe-tag uppercase tracking-tighter">RECIPE ${p.no}</div>
            <div class="icon-animate-wrap">
                <div class="text-[#FF8DA1]"><i data-lucide="${p.icon}" stroke-width="1.5" size="40"></i></div>
            </div>
            <div class="flex-grow w-full text-gray-600 flex flex-col">
                ...
                <div class="flex justify-between items-center mt-auto pt-5 border-t border-dashed border-[#FFD1DC]">
                    ...
                    <div class="w-10 h-10 bg-[#FFD1DC] rounded-xl flex items-center justify-center text-white group-hover:bg-[#FF8DA1]"><i data-lucide="plus" size="20"></i></div>
                </div>
            </div>`;
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // イベント登録
    document.getElementById('menu-open').onclick = () => document.getElementById('mobile-menu').classList.add('translate-x-0');
    document.getElementById('menu-close').onclick = () => document.getElementById('mobile-menu').classList.remove('translate-x-0');
